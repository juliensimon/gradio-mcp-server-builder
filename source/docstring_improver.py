...
class DocstringImprover:
    ...
    def _clean_docstring_syntax(self, docstring: str) -> str:
        ...
        # If the result is empty, too short, or contains suspicious content, use
        # fallback
        suspicious_phrases = [
            "make sure",
            "include exactly",
        ...
        contains_suspicious = any(
            phrase in docstring.lower() for phrase in suspicious_phrases
        )

        if (
            not docstring
            or len(docstring) < 10
            or contains_suspicious
            or "###" in docstring
            or "####" in docstring
            or "returns" not in docstring.lower()
            and "args" not in docstring.lower()
            and len(docstring) > 50
        ):
            return "Function documentation."

        return docstring

    def _create_template_docstring(
        self, function_name: str, signature: str, current_docstring: str
    ) -> str:
        """Create a simple template-based docstring when AI generation fails."""
        # If we have a decent current docstring, use it
        if (
            current_docstring
            and len(current_docstring) > 10
            and "no docstring" not in current_docstring.lower()
        ):
    ...
    def improve_function_docstring(
        ...
        try:
            ...
            cleaned = self.model_config_loader.clean_generated_text(
                generated, "docstring"
            )
            ...
            # If cleaning resulted in a fallback, create a better template-based
            # docstring
            if cleaned == "Function documentation.":
                cleaned = self._create_template_docstring(
                    function_name, signature, current_docstring
                )

            result = (
                cleaned
                if cleaned
                else current_docstring or f"Performs {function_name} operation."
            )
            ...
            return self._create_template_docstring(
                function_name, signature, current_docstring
            )

    def generate_test_prompts(
        self, function_name: str, docstring: str, signature: str
    ) -> List[str]:
        ...
        try:
            ...
            self.logger.debug(
                f"Generated {len(result)} test prompts for {function_name}"
            )
            return result
        except Exception as e:
            self.logger.warning(
                f"Failed to generate test prompts for {function_name}: {e}"
            )
    ...
    def _generate_with_local_model(self, prompt: str) -> str:
        ...
        if self._model is None or self._tokenizer is None:
            ...
            model_name = (
                self.config.local_model or model_config.local_model.default_model
            )
            ...
            try:
                ...
                device_config = self.model_config_loader.get_device_config(
                    self.config.device
                )
                ...
                model_config.local_model.tokenizer_params.copy()
                ...
                if self.config.device == "cpu":
                    self._model = AutoModelForCausalLM.from_pretrained(
                        model_name,
                        torch_dtype=getattr(
                            torch, model_loading_config.get("torch_dtype", "float32")
                        ),
                        device_map=model_loading_config.get("device_map", {"": "cpu"}),
                        low_cpu_mem_usage=model_loading_config.get(
                ...
                self.logger.info(
                    f"Model device: {next(self._model.parameters()).device}"
                )

                # Print improving message now that model is ready
                if (
                    not self._printed_improving_message
                    and not self.config.preserve_docstrings
                ):
                    self.logger.info("Improving docstrings...")
                    self._printed_improving_message = True
