...
if __name__ == "__main__":
    ...
    try:
        with open(config_file, 'r') as f:
            config = json.load(f)
        server_port = config.get("server_port", {port})
    ...
    def _generate_tabbed_interface(self, functions: List[MCPFunction]) -> str:
        ...
        return """# Create the tabbed Gradio interface
...
if __name__ == "__main__":
    ...
    demo.launch(
        ...
        share={share}
            ...
            tab_interfaces=chr(10).join(tab_interfaces),
            port=self.config.port,
            share=self.config.share,
        )
...
def create_agent_interface(mcp_url: str, model_name: str = "{model_name}") -> gr.ChatInterface:
...
def main():
    ...
    try:
        demo, mcp_client = create_agent_interface(mcp_server_url, "{model_name}")
