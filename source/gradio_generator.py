...
class GradioGenerator:
    ...
    def generate_gradio_interface(self, mcp_functions: List[MCPFunction], improved_docstrings: Dict[str, str]) -> str:
        """Generate Gradio interface code."""
        if len(mcp_functions) == 1:
            return self._generate_single_interface(mcp_functions[0], improved_docstrings[mcp_functions[0].name])
        else:
            return self._generate_tabbed_interface(mcp_functions, improved_docstrings)
    
    def _generate_single_interface(self, func: MCPFunction, improved_docstring: str) -> str:
...
'''
    ...
    def _generate_tabbed_interface(self, functions: List[MCPFunction], improved_docstrings: Dict[str, str]) -> str:
