...
from .parser import MCPParser, MCPFunction
...
class GradioMCPBuilder:
                    ...
                    for constant in result.get('module_constants', []):
                        ...
                        if '=' in constant:
                            ...
                            if const_name not in seen_constants:
                                ...
                                self.logger.debug(f"Added unique constant: {const_name}")
                            else:
                                self.logger.debug(f"Skipped duplicate constant: {const_name}")
                    
                    self.logger.debug(f"Found {len(result['mcp_functions'])} MCP functions, {len(result.get('helper_functions', []))} helper functions, and {len(result.get('module_constants', []))} constants in {input_file.name}")
            ...
            with log_step("Improving docstrings", self.logger):
                for func in self.mcp_functions:
                    ...
                    try:
                        ...
                        self.logger.debug(f"Successfully improved docstring for {func.name}")
                    except Exception as e:
                        self.logger.warning(f"Failed to improve docstring for {func.name}: {e}")
                        self.improved_docstrings[func.name] = func.docstring
        ...
        with log_step("Generating test prompts", self.logger):
            for func in self.mcp_functions:
                ...
                try:
                    ...
                    self.logger.debug(f"Generated {len(prompts)} test prompts for {func.name}")
                except Exception as e:
                    self.logger.warning(f"Failed to generate test prompts for {func.name}: {e}")
    ...
    def _generate_server_files(self) -> None:
        ...
        with log_step("Generating server files", self.logger):
            try:
                ...
                server_code = self.server_generator.generate_server(self.mcp_functions, self.helper_functions, self.module_constants, self.improved_docstrings)
    ...
    def _generate_config_file(self) -> None:
        ...
        with log_step("Generating config file", self.logger):
            ...
            self.logger.debug(f"Generated config.json with server port {self.config.port}") 