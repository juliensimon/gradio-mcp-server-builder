...
class MCPParser:
    ...
    def parse_file(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.debug(f"extracted module docstring (length: {len(self.module_docstring)})")
                ...
                if isinstance(node, ast.Import):
                    ...
                    if node.module:
                        if node.names[0].name == '*':
                            ...
                            module_imports.append(f"from {node.module} import {', '.join(names)}")
                ...
                if isinstance(node, ast.Assign):
                    for target in node.targets:
                        if isinstance(target, ast.Name):
                            try:
                                # Extract the complete assignment including multi-line values
                                lines = content.split('\n')
                                start_line = node.lineno - 1  # Convert to 0-based indexing
                                
                                # Find the end of the assignment by looking for the next statement at the same indentation level
                                assignment_indent = len(lines[start_line]) - len(lines[start_line].lstrip())
                                end_line = len(lines)
                                ...
                                for i in range(start_line + 1, len(lines)):
                                    ...
                                    if line.strip():  # Non-empty line
                                        ...
                                        # If we find a line at the same or less indentation that starts a new statement
                                ...
                                if constant_lines:
                                    ...
                                    if constant_def and not constant_def.startswith('#') and '=' in constant_def:
                                        module_constants.append(constant_def)
                                        self.logger.debug(f"found module constant: {target.id} = {len(constant_def)} chars")
                            except Exception as e:
                                self.logger.warning(f"failed to extract constant {target.id}: {e}")
                ...
                if isinstance(node, ast.FunctionDef):
                    ...
                    if has_mcp_decorator:
                        try:
                            ...
                            mcp_func = MCPFunction(node.name, None, docstring, signature)
                            ...
                            self.logger.debug(f"found mcp function: {node.name} at line {node.lineno}")
                        except Exception as e:
                            self.logger.warning(f"failed to process mcp function {node.name}: {e}")
                    else:
                        ...
                        try:
                            ...
                            docstring = ast.get_docstring(node) or ""
                            signature = self._build_signature_string(node)
                            helper_func = MCPFunction(node.name, None, docstring, signature)
                            ...
                            self.logger.debug(f"found helper function: {node.name} at line {node.lineno}")
                        except Exception as e:
                            self.logger.warning(f"failed to process helper function {node.name}: {e}")
            
            self.logger.info(f"successfully parsed {file_path.name}: {mcp_function_count} mcp functions found")
    ...
    def _extract_function_source(self, content: str, node: ast.FunctionDef) -> str:
        ...
        for i in range(node.lineno, len(lines)):
            ...
            if line.strip():  # Non-empty line
                current_indent = len(line) - len(line.lstrip())
                # If we find a line at the same or less indentation that starts a new definition
        ...
        self.logger.debug(f"Extracted {end_line - start_line} lines of source for {node.name}")
        return result
