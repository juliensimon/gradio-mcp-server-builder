...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--model-endpoint",
        type=str,
        help="OpenAI-compatible model endpoint URL"
    )
...
def main() -> int:
    ...
    if args.verbose:
        ...
        for name in ['builder', 'parser', 'generators', 'model_config', 'docstring_improver']:
        ...
        for handler in root_logger.handlers:
            if (hasattr(handler, 'stream') and 
                (handler.stream == sys.stdout or 
                 getattr(handler.stream, 'name', '') == '<stdout>' or
                 handler.__class__.__name__ == 'StreamHandler')):
